...
Functional full-system emulation runs unmodified system software across host
architectures while prioritizing execution speed and tolerating bounded timing
error. QEMU's MTTCG enables parallel vCPU execution, whereas its
instruction-counting mode derives virtual time from guest instructions but
executes vCPUs in a single thread. Cycle-accurate simulators provide finer
timing fidelity at substantially greater computational cost. Many system
workloads use timers to enforce processing deadlines; communication-chip
functional emulation provides a representative example. In this setting,
guest time should reflect instruction progress at a rate calibrated to the
target hardware. A host-driven clock can instead advance beyond the emulated
software's processing progress, producing timeouts absent on the hardware.

SkewKeeper establishes a logical instruction timeline for parallel vCPUs.
A bounded skew window constrains each active vCPU's lead relative to the
minimum active progress. The coordinator periodically publishes
instruction-derived model time, while a shared visible clock applies
momentum-weighted interpolation between publications. Read-time bounds keep
visible time close to the model timeline, mitigating counter plateaus while
maintaining a monotonic, cross-vCPU time base.

SkewKeeper combines a bounded execution window with momentum-based
interpolation to establish a shared, instruction-derived time base for
parallel vCPUs. Linux boot and sustained jitterentropy operation exercise
its timer-dependent software paths, while multicore measurements place its
execution cost alongside icount and MTTCG. Deadline-sensitive communication
workloads retain the expected guest-time behavior as host execution capacity
varies. The same time base extends across runtime transitions between skew
and MTTCG through coordinated visible-time state and interpolation momentum.
...
\begin{figure}[ht]
\centering\includegraphics[width=\linewidth]{figures/design/D1-architecture.pdf}
\caption{Mechanism schematic, not an experimental result.}
\end{figure}
\begin{figure}[ht]
\centering\includegraphics[width=.85\linewidth]{figures/design/D2-window.pdf}
...
\begin{figure}[ht]
\centering\includegraphics[width=\linewidth]{figures/design/D3-time-path.pdf}
